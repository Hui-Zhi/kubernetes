...
func (kl *Kubelet) makeDevices(container *v1.Container) (devices []kubecontainer.DeviceInfo) {
	nvidiaGPULimit := container.Resources.Limits.NvidiaGPU()

	nvidiaGPUPlugin := kl.GetNvidiaGPUPlugin()
	if nvidiaGPULimit.Value() != 0 && nvidiaGPUPlugin != nil {
		nvidiaGPUPaths := nvidiaGPUPlugin.AllocateGPU(container.Name, int(nvidiaGPULimit.Value()))
		...
		if nvidiaGPUPaths != nil {
			...
			for _, path := range nvidiaGPUPaths {
				devices = append(devices, kubecontainer.DeviceInfo{PathOnHost: path, PathInContainer: "/dev/nvidia0", Permissions: "mrw"})
			}
